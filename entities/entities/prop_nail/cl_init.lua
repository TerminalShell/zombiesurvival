...
function ENT:DrawTranslucent()
	local drawowner = MySelf:IsValid() and MySelf:Team() == TEAM_HUMAN and (MySelf:KeyDown(IN_SPEED) or MySelf:TraceLine(256, MASK_SHOT).HitPos:Distance(self:GetPos()) <= 16)
	...
	if drawowner then
		local displayowner = self:GetDTString(1)
		local owner
		if displayowner == "" then
			displayowner = nil
			local deployeruid = self:GetOwnerUID()
			for _, pl in pairs(player.GetAll()) do
				if pl:UniqueID() == deployeruid then
					owner = pl
					break
				end
			end
		...
		cam.Start3D2D(self:GetPos(), ang, 0.05)
			...
			if self:GetMaxRepairs() > 0 then
				...
				draw.SimpleText(math.ceil(repairs), "ZS3DNailHealth", x + wid, y - 1, repairs <= 0 and COLOR_DARKRED or COLOR_GRAY, TEXT_ALIGN_RIGHT, TEXT_ALIGN_TOP)
			end
			...
			if self:GetMaxNailHealth() > 0 then
				...
				draw.SimpleText(math.ceil(self:GetNailHealth()).." / "..math.ceil(self:GetMaxNailHealth()), "ZS3DNailHealth", x + wid / 2, y + hei + 1, colNail, TEXT_ALIGN_CENTER)
			end

			if displayowner or owner and owner:Team() == TEAM_HUMAN then
				--draw.SimpleText(displayowner or owner:Name(), "ZS3D2DFont", 0, y + 8, COLOR_GRAY, TEXT_ALIGN_CENTER)
				draw.SimpleText(displayowner or owner:Name(), "ZS3D2DFont", 0, y + 28, COLOR_GRAY, TEXT_ALIGN_CENTER)
			end
		cam.End3D2D()
	end
end
