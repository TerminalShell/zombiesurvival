...
function ENT:Draw()
	...
	local healthpercent = self:GetTurretHealth() / self:GetMaxTurretHealth()
	...
	cam.Start3D2D(self:LocalToWorld(vScreen), self:LocalToWorldAngles(aScreen), 0.075)
		...
		draw.SimpleText("Integrity: "..math.ceil(healthpercent * 100).."%", "DefaultFontBold", wid * 0.5, hei * 0.5, COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		...
		elseif flash then
			draw.SimpleText("[EMPTY]", "DefaultFontBold", wid * 0.5, hei - 10, COLOR_RED, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
		end
