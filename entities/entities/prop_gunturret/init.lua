...
function ENT:Initialize()
	...
	self:SetMaxTurretHealth(125)
	self:SetTurretHealth(self:GetMaxTurretHealth())
end
...
function ENT:FireTurret(src, dir, numbullets)
	if self:GetNextFire() <= CurTime() then
		...
		if curammo > 0 then
			...
			TURRETOWNER = self:GetObjectOwner()
			TURRET = self
			self:StartBulletKnockback()
			self:FireBullets({Num = numbullets or 1, Src = src, Dir = dir, Spread = Vector(0.05, 0.05, 0), Tracer = 1, Force = 0.1, Damage = 6, Callback = BulletCallback})
	...
	if activator:Team() == TEAM_HUMAN then
		if self:GetObjectOwner():IsValid() then
			...
			if togive > 0 then
				...
				activator:RestartGesture(ACT_ITEM_GIVE)
				self:EmitSound("npc/turret_floor/click1.wav")
				gamemode.Call("PlayerRepairedObject", activator, self, togive * 1.5, self)
			end
		else
			self:SetObjectOwner(activator)
			if not activator:HasWeapon("weapon_zs_turretcontrol") then
				activator:Give("weapon_zs_turretcontrol")
			end
		end
	end
end
...
function ENT:OnPackedUp(pl)
	pl:GiveEmptyWeapon("weapon_zs_turret")
	pl:GiveAmmo(1, "thumper")

	pl.m_PackedTurrets = pl.m_PackedTurrets or {}
	table.insert(pl.m_PackedTurrets, 1, {Health = self:GetTurretHealth(), Ammo = self:GetAmmo()})
...
function ENT:OnTakeDamage(dmginfo)
	...
	if not (attacker:IsValid() and attacker:IsPlayer() and attacker:Team() == TEAM_HUMAN) then
		...
		self:SetTurretHealth(self:GetTurretHealth() - dmginfo:GetDamage())
	end
end
