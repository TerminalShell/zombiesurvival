...
function SWEP:PrimaryAttack()
	...
	if ent:IsValid() then
		...
		local stored = owner.m_PackedMessageBeacons and owner.m_PackedMessageBeacons[1]
		if stored then
			ent.m_Health = stored
			table.remove(owner.m_PackedMessageBeacons, 1)
		end
...
concommand.Add("setmessagebeaconmessage", function(sender, command, arguments)
	...
	if wep:IsValid() and wep:GetClass() == "weapon_zs_beacon" then
		wep.MessageID = math.Clamp(math.floor(tonumber(arguments[1]) or 1), 1, #GAMEMODE.ValidBeaconMessages)
	end
end)
