function MakepOptions()
	...
	local check = vgui.Create("DCheckBoxLabel", Window)
	check:SetText("Play last human music")
	...
	local check = vgui.Create("DCheckBoxLabel", Window)
	check:SetText("Enable rally points")
	check:SetConVar("zs_rallypoints")
	check:SizeToContents()
	list:AddItem(check)
	...
	local check = vgui.Create("DCheckBoxLabel", Window)
	check:SetText("Disable automatic redeeming (active next round)")
	check:SetConVar("zs_noredeem")
	check:SizeToContents()
	list:AddItem(check)

	local check = vgui.Create("DCheckBoxLabel", Window)
	check:SetText("Permanent zombie volunteer (active next round)")
	...
	--list:AddItem(EasyLabel(Window, "Bind a Key to Phase through Barricades"))
	--local DBinder = vgui.Create ( "DBinder", Window)
	--DBinder.OnClick = 
	--list:AddItem(DBinder)
	...
	--list:AddItem(EasyLabel(Window, "Rally point color - Smallest"))
	--local colpicker = vgui.Create("DColorMixer", Window)
	--colpicker:SetAlphaBar(false)
	--colpicker:SetPalette(false)
	--colpicker:SetConVarR("zs_rallycolor_empty_r")
	--colpicker:SetConVarG("zs_rallycolor_empty_g")
	--colpicker:SetConVarB("zs_rallycolor_empty_b")
	--colpicker:SetTall(72)
	--list:AddItem(colpicker)

	--list:AddItem(EasyLabel(Window, "Rally point color - Largest"))
	--local colpicker = vgui.Create("DColorMixer", Window)
	--colpicker:SetAlphaBar(false)
	--colpicker:SetPalette(false)
	--colpicker:SetConVarR("zs_rallycolor_full_r")
	--colpicker:SetConVarG("zs_rallycolor_full_g")
	--colpicker:SetConVarB("zs_rallycolor_full_b")
	--colpicker:SetTall(72)
	--list:AddItem(colpicker)
