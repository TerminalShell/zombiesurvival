CreateClientConVar("zs_bossclass", "[RANDOM]", true, true)
...
function PANEL:SetClassTable(classtable)
	...
	self.NameLabel:SetText(classtable.Name)
...
function PANEL:DoClick()
	if self.ClassTable then
		if self.ClassTable.Boss then
			...
			GAMEMODE:SplitMessage(translate.Format("boss_class_select", self.ClassTable.Name))
		else
...
function PANEL:CreateDescLabels()
	...
	local lines = string.Explode("\n", classtable.Description)
	if classtable.Wave and classtable.Wave > 0 then
		table.insert(lines, 1, "(Unlocked on wave "..classtable.Wave..")")
	end
