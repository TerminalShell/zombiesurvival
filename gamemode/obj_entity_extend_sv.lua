...
function meta:GetDefaultBarricadeHealth()
	local volume = self:OBBMaxs():Distance(self:OBBMins())
	local mass = 2
	local breakable = math.Max( math.Clamp(self:Health(),0,1) * GAMEMODE.BarricadeHealthMin - self:Health(), 0)
	local phys = self:GetPhysicsObject()
	if phys:IsValid() then
		mass = phys:GetMass()
	end

	return math.Clamp(mass * GAMEMODE.BarricadeHealthMassFactor + volume * GAMEMODE.BarricadeHealthVolumeFactor - breakable, GAMEMODE.BarricadeHealthMin, GAMEMODE.BarricadeHealthMax)
end
-- NEW
...
function meta:GhostAllPlayersInMe(timeout, allowrepeat)
	...
	local timnam = "ENABLECOLLISIONS"..self:EntIndex()
	if timer.Exists(timnam) then return end

	self.OldMaterial = self:GetMaterial()
	self.OldCollisionGroup = self:GetCollisionGroup()
	self:SetCollisionGroup(COLLISION_GROUP_DEBRIS_TRIGGER)
	self:SetMaterial("models/spawn_effect")

	if timeout then
		self._EnableCollisionsTimeout = CurTime() + timeout
	else
		self._EnableCollisionsTimeout = nil
	end

	timer.CreateEx(timnam, 0, 0, EnableCollisions, self, 8, TEAM_HUMAN, timnam)
end
...
function meta:ItemCreated()
	...
	timer.SimpleEx(0, CheckItemCreated, self)
end
...
function meta:GetPropsInContraption()
	...
	for _, enttab in pairs(duplicator.Copy(self).Entities) do
		local ent = enttab.Entity
	...
	if attacker:IsPlayer() and attacker:Team() == TEAM_UNDEAD then
		...
		if self:HumanNearby() then
			...
			if attacker.LifeBarricadeDamage ~= nil then
				attacker.LifeBarricadeDamage = attacker.LifeBarricadeDamage + dmg
			end
...
function meta:DamageNails(attacker, inflictor, damage, dmginfo)
	...
	col.g = math.Min((self:GetBarricadeHealth()/self:GetMaxBarricadeHealth())*255,255)
	col.b = col.g
	self:SetColor(col)
