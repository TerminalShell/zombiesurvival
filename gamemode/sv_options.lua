...
GM.EndWaveHealthBonus = 5 /*CreateConVar("zs_endwavehealthbonus", "5", FCVAR_ARCHIVE + FCVAR_NOTIFY, "Humans will get this much health after every wave. 0 to disable."):GetInt()
...
GM.MaxDroppedItems = CreateConVar("zs_maxdroppeditems", "15", FCVAR_ARCHIVE + FCVAR_NOTIFY, "Maximum amount of dropped items. Prevents spam or lag when lots of people die."):GetInt()
...
cvars.AddChangeCallback("zs_nailhealthperrepair", function(cvar, oldvalue, newvalue)
	GAMEMODE.NailHealthPerRepair = tonumber(newvalue) or 10
end)

GM.BarricadeHealthMax = CreateConVar("zs_barricadehealthmax", "1200", FCVAR_ARCHIVE + FCVAR_NOTIFY, "Nailed props can not have more than this health."):GetInt()
cvars.AddChangeCallback("zs_barricadehealthmax", function(cvar, oldvalue, newvalue)
	GAMEMODE.BarricadeHealthMax = tonumber(newvalue) or 1200
end)
...
GM.BarricadeHealthMin = CreateConVar("zs_barricadehealthmin", "750", FCVAR_ARCHIVE + FCVAR_NOTIFY, "Nailed props can not have less than this health."):GetInt()
cvars.AddChangeCallback("zs_barricadehealthmin", function(cvar, oldvalue, newvalue)
	GAMEMODE.BarricadeHealthMin = tonumber(newvalue) or 600
end)

GM.BarricadeHealthMassFactor = math.ceil(100 * CreateConVar("zs_barricadehealthmassfactor", "12", FCVAR_ARCHIVE + FCVAR_NOTIFY, "Nailed props receive this much health per kg."):GetFloat()) * 0.01
cvars.AddChangeCallback("zs_barricadehealthmassfactor", function(cvar, oldvalue, newvalue)
	GAMEMODE.BarricadeHealthMassFactor = tonumber(newvalue) or 12
end)

GM.BarricadeHealthVolumeFactor = math.ceil(100 * CreateConVar("zs_barricadehealthvolumefactor", "5", FCVAR_ARCHIVE + FCVAR_NOTIFY, "Nailed props receive this much health per square unit."):GetFloat()) * 0.01
cvars.AddChangeCallback("zs_barricadehealthvolumefactor", function(cvar, oldvalue, newvalue)
	GAMEMODE.BarricadeHealthVolumeFactor = tonumber(newvalue) or 5
end)
